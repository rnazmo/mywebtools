...
- [x] **Must1**: 音通知（フェーズ切り替え時にビープ音を鳴らす）
...
- [x] **Must3**: フェーズごとのページ背景色変更
  - 集中タイム・休憩タイムのどちらなのかを瞬時に判別できるようにする
  - 変更箇所の案: ページ全体の背景色をうっすら変える（カードなど部分的な変更ではなく全体）
  - 色の案: 集中タイム = 青系、休憩タイム = 緑系（しっくりくる色は試行錯誤で決める）
  - Tailwind の動的クラス切り替えで実装する？
  - このポモドーロタイマーはあくまでツールの１つである。今後他のツールも色付けするので、その際に統一感のある色付けができるようにすること。
  - ボタンとの色の兼ね合いも注意すること
- [x] **Must4**: 手動フェーズ切り替え
  - 「Skip ボタン」案
  ...
  - 「Carousel」案
    - shadcn/ui を眺めていて，Carousel が良い感じだったので使えそうかも
      - [Carousel - shadcn／ui](https://ui.shadcn.com/docs/components/radix/carousel)
    - キーボードの ← → でスムーズに切り替えられそうで，良い感じ。
  - ADR を書くこと。案：「手動フェーズ切り替えの UI 設計」
